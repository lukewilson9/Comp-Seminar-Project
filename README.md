...
python -m ipykernel install --user --name=pipeline --display-name "Python (myenv)"
...
Note: All of my hyperparameters from my tuning runs are stored for each subsystem and the combiner in .txt files in the repository, starting with names for each subsystem or the combiner, respectively.
